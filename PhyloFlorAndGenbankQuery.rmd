...
specieslist <- as.character(mydata[,1][mydata[,5] != "S"])
specieslist


specieslist <- paste(specieslist, "[Organism]", sep='')
specieslist

#specieslist <- paste(specieslist, collapse=' OR ')
#specieslist

specieslist <- paste(specieslist, " AND internal transcribed spacer", sep='')
specieslist

library(rentrez)

Query1 <- list()
for (i in 1:length(specieslist)) Query1[[i]] <- entrez_search(db="nuccore", term=specieslist[i])

Query1

ITS <- matrix(0,471,2)
ITS[,1] <- as.character(mydata[,1][mydata[,5] != "S"])
for (i in 1:471) ITS[i,2] <- paste(Query1[[i]][1])
ITS
